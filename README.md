...
برای بخش یادگیری ماشین، وظایف به‌گونه‌ای تقسیم شده‌اند که هر عضو یک بخش مستقل، قابل ارائه و تقریباً هم‌حجم داشته باشد. همچنین بخش‌های اصلی علی و بنیامین به کار لیلا و تینا وابسته نیستند تا در صورت تأخیر یا عدم تکمیل بخش‌های دیگر، مسیر اصلی پروژه دچار مشکل نشود.

#### علی: خوشه‌بندی اصلی و توصیه‌گر پایه
...
* آماده‌سازی داده معتبر برای خوشه‌بندی از فایل `cleaned_step3_geo.parquet`
* محدود کردن تحلیل مکانی به `UTM zone 39S`
* انتخاب ویژگی‌های اصلی برای خوشه‌بندی
* ساخت نسخه لگاریتمی قیمت هدف در صورت نیاز
* scale کردن ویژگی‌ها
* اجرای KMeans با ۱۰ خوشه
* رسم scatter plot خوشه‌ها
* رسم مرکز هر خوشه روی نمودار
* ساخت جدول خلاصه خوشه‌ها
* تفسیر خوشه‌ها برای کاربرد توصیه‌گر

فیلتر پیشنهادی برای داده خوشه‌بندی:
...
نقاط مرکز خوشه‌ها
جدول خلاصه هر خوشه
تحلیل کاربرد هر خوشه در توصیه‌گر
```

#### لیلا: انتخاب ویژگی و انتخاب مقدار مناسب k

فایل کاری:

```text
notebooks/work/clustering_leila_wcss.ipynb
```
...
* آماده‌سازی مستقل داده خوشه‌بندی از فایل `cleaned_step3_geo.parquet`
* اعمال همان فیلتر پایه خوشه‌بندی
* بررسی و مقایسه چند مجموعه ویژگی برای clustering
* اجرای KMeans برای kهای ۱ تا ۲۰
* محاسبه WCSS برای هر مقدار k
* رسم نمودار elbow
* بررسی اینکه elbow واضح است یا نه
* انتخاب مقدار مناسب k با استدلال
* تحلیل اثر تعداد و نوع ویژگی‌ها بر کیفیت خوشه‌بندی
* توضیح خطر نفرین ابعاد در انتخاب ویژگی‌های زیاد

مجموعه ویژگی‌های پیشنهادی برای مقایسه:
...
تحلیل elbow
k پیشنهادی
مقایسه feature setها
توضیح نهایی برای انتخاب ویژگی‌ها
```

#### تینا: خوشه‌بندی با DBSCAN

فایل کاری:

```text
notebooks/work/clustering_tina_dbscan.ipynb
```
...
* آماده‌سازی مستقل داده مناسب برای DBSCAN از فایل `cleaned_step3_geo.parquet`
* اعمال فیلترهای معتبر قیمت و جغرافیا
* محدود کردن تحلیل مکانی به `UTM zone 39S`
* انتخاب ویژگی‌های UTM و قیمت هدف
* ساخت نسخه لگاریتمی قیمت هدف در صورت نیاز
* scale کردن ویژگی‌ها
* تست چند مقدار برای `eps`
* تست چند مقدار برای `min_samples`
* تلاش برای رسیدن به ۳ کلاستر معنادار
* رسم scatter plot نهایی
* تحلیل نقاط noise با label برابر `-1`
* توضیح اثر هایپرپارامترهای DBSCAN
...
```text
جدول تست چند مقدار eps و min_samples
خروجی نهایی با ۳ کلاستر معنادار
scatter plot نهایی
تحلیل نقاط noise
توضیح اثر eps
توضیح اثر min_samples
مقایسه کوتاه DBSCAN و KMeans
```

#### بنیامین: پیش‌بینی قیمت

فایل کاری:

```text
notebooks/work/prediction_benyamin.ipynb
```
...
* آماده‌سازی داده معتبر برای مدل‌سازی قیمت از فایل `cleaned_step3_geo.parquet`
* تعریف متغیر هدف
* ساخت نسخه لگاریتمی هدف در صورت نیاز
* جلوگیری از نشت متغیر هدف
* انتخاب ویژگی‌های ورودی مناسب
* مدیریت مقدارهای گم‌شده در ویژگی‌های مدل
* encoding ویژگی‌های دسته‌ای
* تقسیم داده به train، validation و test
* آموزش مدل پایه
* آموزش یک مدل بهبودیافته
* انتخاب مدل نهایی فقط با validation
* ارزیابی نهایی فقط روی test
* گزارش معیارهای ارزیابی
* تحلیل خطاهای مدل
...
در گزارش نهایی، پیش‌بینی‌ها باید به مقیاس اصلی قیمت برگردانده شوند.

ستون‌هایی که نباید به‌عنوان feature استفاده شوند، چون باعث leakage می‌شوند:
...
MSE
تحلیل خطا
توضیح محدودیت‌های مدل
```

## منطق استقلال بخش‌ها در یادگیری ماشین

برای کاهش ریسک پروژه، هر عضو بخش خودش را از فایل نهایی preprocessing شروع می‌کند:

```text
cleaned_step3_geo.parquet
```

وابستگی‌ها به این شکل تعریف شده‌اند:

```text
علی: مستقل از لیلا و تینا
بنیامین: مستقل از همه اعضای بخش clustering
لیلا: مستقل، اما قابل ادغام با تحلیل KMeans علی
تینا: مستقل، اما قابل مقایسه با خروجی KMeans علی
```

بنابراین اگر لیلا یا تینا به هر دلیلی بخش خود را کامل نکنند، بخش‌های اصلی زیر همچنان قابل تحویل هستند:

```text
KMeans اصلی و توصیه‌گر پایه توسط علی
پیش‌بینی قیمت توسط بنیامین
```

اما در صورت تکمیل کار لیلا و تینا، کیفیت تحلیل clustering و امتیاز بخش یادگیری ماشین بهتر خواهد شد.

## نکات اجرایی برای بخش ML

* همه افراد باید از فایل `cleaned_step3_geo.parquet` استفاده کنند.
...
سؤال ۲: کامل شده ✅
سؤال ۳: کامل شده ✅
سؤال ۴: مسئول بنیامین
سؤال ۵: مسئول بنیامین
سؤال ۶: مسئول بنیامین، شروع شده اما کامل نشده
سؤال ۷: مسئول لیلا
...
```text
فرضیه ۱: کامل شده و commit شده ✅
```

### یادگیری ماشین

```text
04_clustering.ipynb: در حال شروع
05_prediction.ipynb: در انتظار شروع
```
